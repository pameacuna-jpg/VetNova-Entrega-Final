...
class GlobalExceptionHandlerTest {
    ...
    void handleServiceUnavailable_debeRetornar503() {
        ServiceUnavailableException ex = new ServiceUnavailableException("Mascotas no disponible");
        ...
        assertEquals("Mascotas no disponible", response.getBody().getMessage());
    }
    ...
    void handleValidationExceptions_debeRetornar400ConPrimerMensaje() {
        ...
        org.springframework.validation.FieldError fieldError =
                new org.springframework.validation.FieldError("fichaClinicaRequestDTO", "idMascota", "El ID de la mascota es obligatorio");
        ...
        assertNotNull(response.getBody());
        assertEquals("El ID de la mascota es obligatorio", response.getBody().getMessage());
    }
